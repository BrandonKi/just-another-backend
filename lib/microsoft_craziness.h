#ifndef MICROSFT_CRAZINESS_H
#define MICROSFT_CRAZINESS_H
...
#define MSC_DEFER const auto& CONCAT(defer__, __LINE__) = ExitScopeHelp() + [&]()
...
inline wchar_t *concat(wchar_t const *a, wchar_t const *b, wchar_t const *c = nullptr, wchar_t const *d = nullptr) {
...
inline bool visit_files_w(wchar_t *dir_name, Version_Data *data, Visit_Proc_W proc) {
...
inline void win10_best(wchar_t *short_name, wchar_t *full_name, Version_Data *data) {
...
inline void win8_best(wchar_t *short_name, wchar_t *full_name, Version_Data *data) {
...
inline void find_windows_kit_root(FindResult *result) {
    ...
    if (windows8_root) {
        MSC_DEFER { free(windows8_root); };
        
        auto windows8_lib = concat(windows8_root, L"Lib");
        MSC_DEFER { free(windows8_lib); };
...
inline void find_visual_studio_by_fighting_through_microsoft_craziness(FindResult *result) {
    ...
    if (hr != 0)  return;
    MSC_DEFER { config->Release(); };

    IEnumSetupInstances *instances = NULL;
    hr = config->EnumInstances(&instances);
    if (hr != 0)     return;
    if (!instances)  return;
    MSC_DEFER { instances->Release(); };
    ...
    for (int i = 0; i < NUM_VERSIONS; i++) {
        ...
        MSC_DEFER { free(vcruntime_filename); };
